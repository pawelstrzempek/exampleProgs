...
root -l
.L graphDraw.C++
grapher("/path1/file1,/path2/file2,...","Graph title","X axis title", "Y axis title")
*/
//----------------------------------------------------------------------------------------------------------
...
using namespace std;
std::vector <float> Xtab;
std::vector <float> Ytab;
std::vector <float> YtabErr;
std::vector <float> XtabErr;
...
int readIn(string FileName){
  ...
  while(fgets(line, 80, pFile) != NULL)
  {
         ...
         sscanf (line, "%f\t%f\t%f\t%f", &x,&y,&x_Err,&y_Err);
         Xtab.push_back(x);
         Ytab.push_back(y);
	 YtabErr.push_back(y_Err);
	 XtabErr.push_back(x_Err);
  }
...
*/
int xgrapher(string File, string Title="", string xAxisTitle = "", string yAxisTitle = ""){
  ...
  // return 0;
   ...
   TGraphErrors **gr = new TGraphErrors*[FileQuantity];
   ...
   for (int k = 0; k < FileQuantity ; k++){
    ...
    // }
     ...
     {
       x[i] = Xtab.front();
       Xtab.erase(Xtab.begin(),Xtab.begin()+1);
       y[i] = Ytab.front();
       Ytab.erase(Ytab.begin(),Ytab.begin()+1);
       xErr[i] = XtabErr.front();
       XtabErr.erase(XtabErr.begin(),XtabErr.begin()+1);;
       yErr[i] = YtabErr.front();
       YtabErr.erase(YtabErr.begin(),YtabErr.begin()+1);;
     }
     gr[k] = new TGraphErrors(n,x,y,xErr,yErr);
     gr[k]->SetLineColor(k+1);
     gr[k]->SetLineWidth(2);
     gr[k]->SetLineStyle(1);//3
     gr[k]->SetMarkerStyle(8);
     gr[k]->SetMarkerSize(1.2);
